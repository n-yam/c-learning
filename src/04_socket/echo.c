...
int main(){
	...
	server_addr.sin_port = htons(8000); // htons maans "Host to network short" (=convert to Big Endian)
	...
	do {
		...
		ssize_t write_size = write(client_socket, write_buffer, (size_t) read_size);
		if (write_size < 0) {
			perror("FAILED TO RECEIVE");
			exit (EXIT_FAILURE);
		}
